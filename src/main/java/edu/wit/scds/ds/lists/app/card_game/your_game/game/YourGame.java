...
// @formatter:off
// NOTE you will modify this code - talk to me if you have any questions
// NOTE some of these methods have been fully modified - some are still Top This! logic
// @formatter:on
...
import static edu.wit.scds.ds.lists.app.card_game.standard_cards.pile.Pile.newTransferPile ;
 ...
 * <p>
 * This is the main driver for the game of Top This. It supports 3 or more players. Players take turns using a
 ...
 * Rules:
 * <ul>
 * <li>use 1 or more decks of standard playing cards (52 cards per deck, 4 suits, 13 ranks, no jokers) as
 * specified by the players
 * <li>3 or more players are each dealt the same number of cards (as specified by the players), one at a time,
 * in rotation
 ...
 * <li>if there is a single highest card, whichever player played it takes all the cards and saves them as a
 * meld
 * <li>if two or more players played the highest card, there is no winner for that round and the cards are
 * placed face down on the discard pile
 * </ul>
 * <li>
 * <li>a player wins the round when their card is the highest played in that round
 * <li>if there is no winner in a round (2 or more players have the highest card), the cards are discarded
 * <li>the cards won during a round are given to the player with the highest card and saved as a meld
 ...
 * </ul>
 * <p>
 * NOTE You must rename this class to whatever your game is called. If the name of the game begins with a
 * number, spell out the number (can't start a class name with a digit). Replace the comments above describing
 * my game with appropriate comments for yours.
 *
 ...
 * @version 2.2 2026-03-26 replace anonymous pile with factory invocation
 *
 * @author Your Name
 *
...
public final class YourGame
    ...
    /**
     * This is the top-level driver for the game.
     *
     * @param args
     *     -unused-
     */
    public static void main( final String[] args )
        ...
        try ( final Scanner input = new Scanner( System.in ) ; )
            ...
            final YourGame game = new YourGame( input ) ;
            ...
            while ( game.running )
                ...
                final String playAgain = game.promptForLine( "Play again?" ) ;
    ...
    private void configureCards()
        ...
        do
            ...
            this.numberOfDecks = promptForInt( "%nHow many decks (minimum %,d)?",
                                               Math.max( ( this.numberOfPlayers / Deck.CARD_COUNT_IN_A_DECK )
                                                         + 1,
    ...
    private void configureCardsPerHand()
        ...
        final int maximumCardsPerHand = this.stock.cardCount() / this.numberOfPlayers ;
    ...
    private void getCardsFromDecks()
        ...
        // the decks
        // NOTE alternative: determine minimum # of decks based upon the # of hands and # of cards/hand
        // @formatter:on
        ...
        for ( int i = 1 ; i <= this.numberOfDecks ; i++ )
            ...
            newDeck.moveCardsToBottom( newCards.removeAllMatchingCards( joker )
                                               .revealAll() ) ;
    ...
    private void run()
        ...
        for ( this.roundNumber = 1 ; this.roundNumber <= this.numberOfRounds ; this.roundNumber++ )
            ...
            // @formatter:off
            // make new pile to hold the cards played during this round cards will be added face down
            // after all players have taken their turn, the cards will all be turned face up
            // @formatter:on

            final Pile cardsInPlay = newTransferPile().setDefaultFaceDown() ;
    ...
    private Card promptForCard( final String prompt,
        ...
        do
            ...
            if ( input.length() == 1 )
                ...
                // see if it's a suit
                suit = Suit.interpretDescription( input ) ;
                if ( suit != null )
                    {

                    // got it
                    return new Card( Rank.NA,
                                     suit ) ;
                    }
                
                // see if it's a rank
                rank = Rank.interpretDescription( input ) ;
                if ( rank != null )
                    {
            ...
            if ( ( input.length() == 1 ) || ( input.length() > 2 ) )
                ...
                System.out.printf( "%nUnrecognized or invalid response: \"%s\"; valid responses must have 1 or 2 characters, please try again ('?' for help)",
                                   input ) ;
            ...
            final String rankElement = input.substring( 0,
                                                        1 ) ;
            final String suitElement = input.substring( 1,
                                                        2 ) ;
