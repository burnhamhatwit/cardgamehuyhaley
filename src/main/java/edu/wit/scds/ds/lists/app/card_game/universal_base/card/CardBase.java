 ...
 *     </ul>
 * @version 3.0 2025-11-02 rename from {@code UniversalBaseCard} to {@code CardBase} - less of a mouthful
 * @version 3.1 2025-11-11 add {get,set}FaceUpTextLocked()
 * @version 3.2 2025-11-19 add template card support
 */
public abstract class CardBase
        implements Comparable<CardBase>
    {
    ...
    /**
     * default lack of decoration for cards where [0] is applied immediately before the face text and [1]
     * immediately following
     */
    protected static final String[] DEFAULT_NO_DECORATION = { "", "" } ;

    /**
     * default decoration for permanent cards where [0] is applied immediately before the face text and [1]
     * immediately following
     */
    protected static final String[] DEFAULT_PERMANENT_DECORATION = { "", "" } ;

    /**
     * default decoration for template cards where [0] is applied immediately before the face text and [1]
     * immediately following
     */
    protected static final String[] DEFAULT_TEMPLATE_DECORATION = { "├", "┤" } ;

    /**
     * default decoration for temporary cards where [0] is applied immediately before the face text and [1]
     * immediately following
     */
    ...
    /**
     * default state for decoration for permanent cards where [0] is applied immediately before the face text
     * and [1] immediately following
     */
    protected static String[] defaultPermanentCardDecoration = DEFAULT_PERMANENT_DECORATION ;

    /**
     * default state for decoration for template cards where [0] is applied immediately before the face text
     * and [1] immediately following
     */
    protected static String[] defaultTemplateCardDecoration = DEFAULT_TEMPLATE_DECORATION ;

    /**
     * default state for decoration for temporary cards where [0] is applied immediately before the face text
     * and [1] immediately following
     */
    ...
    /**
     * permanent, template (used during deck instantiation), temporary (used for lookup/matching)
     */
     ...
     * <p>
     * Note: this constructor is typically used by a deck to create permanent cards from templates in its
     * constructor or during game play for cloning a card which should be temporary for matching, lookup, etc.
     *
    ...
    /**
     * create a copy of this card, typically temporary if we're permanent, permanent if we're template
     * <p>
     * only {@code CardBase} fields are copied; the subclass is responsible for copying everything else
     *
     * @param sourceCard
     *     the card to clone
     * @param cardPersistence
     ...
     *
     * @return the text to display when the card is face down - {@code null} indicates to use the default
     */
    ...
    public static String[] getTemplateDecoration()
        ...
        return Arrays.copyOf( defaultTemplateCardDecoration,
                              defaultTemplateCardDecoration.length ) ;
    ...
    /**
     * Compare two cards to see if they match, which may be different from them being {@code equal()}
     * <p>
     * The game or {@code Pile} class may require this method to behave differently from {@code equals()} or
     * {@code compareTo()} which will be implemented in the game-specific Card class.
     * <p>
     * Note: this implementation uses {@code equals()} to perform the match functionality which may be
    ...
    public boolean matches( final CardBase otherCard )
        ...
        // delegate to equals() - game-specific card should override if necessary
    ...
    public static Orientation setDefaultOrientation( final Orientation newOrientation )
        ...
        Objects.requireNonNull( newOrientation,
                                "newOrientation" ) ;
     ...
     * <p>
     * Note: decks (the classes which instantiate the cards for game play) are the only classes expected to
    ...
    public static Persistence setDefaultPersistence( final Persistence newPersistence )
        ...
        Objects.requireNonNull( newPersistence,
                                "newPersistence" ) ;
    ...
    /**
     * ensure that a persistence is supplied and applicable to a card (is one of {@code TEMPORARY},
     * {@code PERMANENT}), or {@code TEMPLATE}
     *
     * @param aPersistence
    ...
    private static void validatePersistence( final Persistence aPersistence )
        ...
        Objects.requireNonNull( aPersistence,
                                "aPersistence" ) ;
    ...
    public CardBase setFaceDownText( final String newFaceDownText )
        ...
        Objects.requireNonNull( newFaceDownText,
                                "newFaceDownText" ) ;
    ...
    public CardBase setFaceUpText( final String newFaceUpText )
        ...
        Objects.requireNonNull( newFaceUpText,
                                "newFaceUpText" ) ;
    ...
    /**
     * set the decoration text for permanent cards where [0] is the left decoration and [1] is the right
     *
    ...
    /**
     * set the decoration text for template cards where [0] is the left decoration and [1] is the right
     *
    ...
    public static String[] setTemplateDecoration( final String[] newTemplateDecoration )
        ...
        // the array must be non-null, length 2, and contain non-null references
        Objects.requireNonNull( newTemplateDecoration,
                                "newTemplateDecoration" ) ;
        ...
        Objects.requireNonNull( newTemplateDecoration[ 0 ],
                                "newTemplateDecoration[ 0 ]" ) ;
        Objects.requireNonNull( newTemplateDecoration[ 1 ],
                                "newTemplateDecoration[ 1 ]" ) ;

        final String[] priorDecoration = defaultTemplateCardDecoration ;

        // make a copy of the array for safety
        defaultTemplateCardDecoration = Arrays.copyOf( newTemplateDecoration,
                                                       newTemplateDecoration.length ) ;
    ...
    /**
     * set the decoration text for temporary cards where [0] is the left decoration and [1] is the right
     *
    ...
    public static String[] setTemporaryDecoration( final String[] newTemporaryDecoration )
        ...
        Objects.requireNonNull( newTemporaryDecoration[ 0 ],
                                "newTemporaryDecoration[ 0 ]" ) ;
        Objects.requireNonNull( newTemporaryDecoration[ 1 ],
                                "newTemporaryDecoration[ 1 ]" ) ;

        final String[] priorDecoration = defaultTemporaryCardDecoration ;

        // make a copy of the array for safety
        defaultTemporaryCardDecoration = Arrays.copyOf( newTemporaryDecoration,
                                                        newTemporaryDecoration.length ) ;
    ...
    private void setDecoration()
        ...
        System.arraycopy( switch ( this.persistence )
            ...
            case Persistence.PERMANENT -> defaultPermanentCardDecoration ;
            case Persistence.TEMPLATE -> defaultTemplateCardDecoration ;
            case Persistence.TEMPORARY -> defaultTemporaryCardDecoration ;
            default -> throw new IllegalStateException( this.persistence.toString() ) ;
