 ...
 *     <ul>
 *     <li>change to {@code abstract} class so games must be built with appropriate subclasses of {@code Pile}
 *     <li>remove no-arg constructor
 *     </ul>
 * @version 3.0 2025-03-30
 *     <ul>
 *     <li>switch from {@code ArrayList} to {@code LinkedList} to improve efficiency to add/remove at both
 *     ends of the list (top and bottom) with O(1) efficiency
 ...
 *     </ul>
 * @version 6.0 2025-11-02 rename from {@code UniversalBasePile} to {@code PileBase} - less of a mouthful
 * @version 6.1 2025-11-19
 *     <ul>
 *     <li>add template card support
 *     <li>prevent clearing a pile unless it only contains temporary cards
 *     </ul>
 * @version 6.2 2025-11-19 add support for verification of decks that they have the correct contents,
 *     typically at the end of a game to determine if any cards have been lost
 */
public abstract class PileBase
        implements Iterable<CardBase>
    {
    ...
    /**
     * restrict to permanent, template, temporary, or accept any - can only be set during instantiation
     */
    ...
    /**
     * initialize this pile with added cards required to be compatible with the specified persistence
     *
     * @param persistenceSetting
     *     specify whether this pile will only accept permanent, template, temporary, or any cards
     *
     * @since 3.0
     */
     ...
     * @since 3.0
     */
    public CardBase getCardAt( final int position )
            throws NoCardsException
        {
    ...
    public CardBase getCardLike( final CardBase likeCard )
        ...
        Objects.requireNonNull( likeCard,
                                "likeCard" ) ;
     ...
     * Caution: the returned card should only be in one pile at any time
     * <p>
     * Note: there is no corresponding convenience method to retrieve the bottom card since that's usually
     * cheating and we can't condone that 8~)
     *
     * @return the top card
     *
     * @throws NoCardsException
     ...
     * <p>
     * This count may be different than would be determined by testing cards for equality, for instance, if
     * matching only rank or suit.
     *
     * @param targetCard
    ...
    public int matchCount( final CardBase targetCard )
        ...
        Objects.requireNonNull( targetCard,
                                "targetCard" ) ;
    ...
    public PileBase moveCardsToBottom( final PileBase otherCards )
        ...
        Objects.requireNonNull( otherCards,
                                "otherCards" ) ;

        // add the cards to the bottom of our pile
        this.cards.addAll( nextBottomPosition(),
                           otherCards.cards ) ;
    ...
    public PileBase moveCardsToTop( final PileBase otherCards )
        ...
        this.cards.addAll( nextTopPosition(),
                           otherCards.cards ) ;
    ...
    public PileBase removeAll()
        ...
        final PileBase allRemovedCards = new PileBase()
            {} ;
        allRemovedCards.setAcceptablePersistence( UNRESTRICTED )
                       .setDefaultOrientation( AS_IS ) ;

        // move (copy then delete all) all the cards from our list into the the temporary pile
    ...
    public PileBase removeAllMatchingCards( final CardBase lookupCard,
        ...
        Objects.requireNonNull( lookupCard,
                                "lookupCard" ) ;
        Objects.requireNonNull( removedCards,
                                "removedCards" ) ;
     ...
     *     if the pile is empty
     * @throws IndexOutOfBoundsException
     *     if the specified position is negative or greater than or equal to the number of cards in the pile
     */
    public CardBase removeCardAt( final int position )
     ...
     * <p>
     * Note: there is no corresponding convenience method to remove the bottom card since that's usually
     * cheating and we can't condone that 8~)
     *
     * @return the top card
     *
    ...
    /**
     * set the persistence requirements for this pile (permanent, template, temporary, unrestricted)
     *
     * @param persistenceSetting
    ...
    public PileBase setAcceptablePersistence( final Persistence persistenceSetting )
        ...
        Objects.requireNonNull( persistenceSetting,
                                "persistenceSetting" ) ;
    ...
    private void prepareCardToAdd( final CardBase newCard )
        ...
        Objects.requireNonNull( newCard,
                                "newCard" ) ;

        // check persistence card's persistence must match ours
     ...
     */
    private class CardBaseIterator
